...
export default function SubjectSelector({ childId, childName, locale, progressMap }: SubjectSelectorProps) {
  ...
  const taalSubjects = SUBJECTS.filter(s => s.taalGroup);
  const otherKernSubjects = SUBJECTS.filter(s => s.category === 'kern' && !s.taalGroup);
  ...
  const renderKernCard = (subject: typeof SUBJECTS[0]) => {
    ...
    return (
      <Link key={subject.id} href={href} className="group">
        <div className="bg-gradient-to-br from-sky-400 to-sky-600 rounded-2xl shadow-lg hover:shadow-2xl transition-all p-6 border-4 border-transparent hover:border-sky-300 transform hover:scale-105 cursor-pointer text-white">
          <div className="text-center">
            {subject.taalGroup && (
              <div className="text-xs font-semibold bg-white/20 rounded-full px-2 py-0.5 inline-block mb-2">📖 Taal</div>
            )}
            <div className="text-5xl mb-3">{subject.icon}</div>
            <h3 className="text-xl font-bold mb-2">{getSubjectLabel(subject)}</h3>
  ...
  const renderZaakCard = (subject: typeof SUBJECTS[0]) => {
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        {/* Alle 4 kernvakken gelijke grootte */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
          {taalSubjects.map(renderKernCard)}
          {otherKernSubjects.map(renderKernCard)}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {zaakSubjects.map(renderZaakCard)}
        </div>
      </div>
    </div>
  );
